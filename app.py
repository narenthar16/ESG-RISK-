...
if gold is not None:
    ...
    with tab1:
        ...
        with col2:
            fig = px.scatter(gold, x="total_esg_score", y="risk_score",
    ...
    with tab2:
        ...
        with col1:
            country_filter = st.selectbox("Country",
                ["All"] + sorted(gold["country"].unique()))
        with col2:
            sector_filter = st.selectbox("Sector",
                ["All"] + sorted(gold["sector"].unique()))
        ...
        fig = go.Figure(go.Indicator(
            mode="gauge+number", value=float(row["risk_score"]),
            title={"text": f"{ticker} Risk Score"},
            gauge={"axis": {"range":[0,100]}, "bar": {"color":"#2c3e50"},
                   "steps": [{"range":[0,35],"color":"#27ae60"},
        ...
        st.dataframe(filtered[show_cols].sort_values("risk_score"),
                     use_container_width=True)

    with tab3:
        if sector is not None:
            fig = px.bar(sector.sort_values("avg_esg_score"),
                x="avg_esg_score", y="sector", orientation="h",
            ...
            with col2:
                fig = px.scatter(sector,
                    ...
                    size="n_companies", color="avg_carbon",
    ...
    with tab4:
        if metrics is not None:
            ...
            if acc:
                ...
                train = float(acc.get('train_accuracy',0))
                test  = float(acc.get('test_accuracy',0))
                col3.metric("Overfitting Gap",
                            f"{abs(train-test)*100:.2f}%")
            ...
            if len(cm) > 0:
                ...
                fig = px.imshow(cm, text_auto=True, x=labs, y=labs,
